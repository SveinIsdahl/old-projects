...
document.getElementById("button").addEventListener("click", function () {
    if (state == "static") {
        state = "elastic";
        document.getElementById("button").innerHTML = "Elastic";
    }
    else {
        state = "static";
        document.getElementById("button").innerHTML = "Static";
    }
});
...
function doBallsOverlap(ball, target) {
    ...
    if (
        ...
        vecFunc.avstandPunkt(ball, target) < (ball.r + target.r)
    ) {
        return true;
    }
    else {
...
for (let i = 0; i < 5; i++) {
    ball[i] = new Ball(
        //i * Math.random() * 0.01
        30
        , Math.random() * (width - 25) + 10, Math.random() * (height - 25) + 10, i);
}
...
Lag MASSE sliders og knapper (som i tegne.html)
*/